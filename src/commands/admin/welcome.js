  ...
  handler: async (client, message, args) => {
    try {
      ...
      if (!args.text) {
        ...
        return message.reply(msg('📝', `Поточний текст привітання:\n${currentText}`));
      }
      ...
      return message.reply(msg('✅', `Текст привітання оновлено! \n ${response}`));
    } catch (error) {
      console.error('Welcome text error:', error);
      message.reply(err('Помилка при встановленні тексту привітання'));
    }
  }
};